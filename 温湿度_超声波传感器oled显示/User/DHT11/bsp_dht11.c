...
void DHT11_TIM3_Configuration(void)
{
    ...
    TIM_TimeBaseStructure.TIM_Period = 5 - 1; // 定时50us
    TIM_TimeBaseStructure.TIM_Prescaler = 840 - 1; // 时钟频率为84MHz，分频系数为8400，计数频率为10kHz
...
void DHT11_func(void)
{
	...
	{
		DEBUG("Read DHT11 ERROR!\r\n");
	}
	
	/*调用DHT11_Read_TempAndHumidity读取温湿度，若成功则输出该信息*/
	humi = DHT11_Data.humi_int+(float)(DHT11_Data.humi_deci)/10;
